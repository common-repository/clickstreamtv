...
<hr/>
<p>
   <label for="<?php echo $this->get_field_id( 'cstv_playlist' ); ?>"><?php _e('Select a playlist:', 'cstv_playlist'); ?></label> 
   <select id="<?php echo $this->get_field_id( 'cstv_playlist' );?>" name="<?php echo $this->get_field_name( 'cstv_playlist' );?>" class="widefat" style="width:100%;">
       <?php $playlists = explode('|',urldecode($playlists)); foreach($playlists as $v) : list($id,$title) = explode('::',$v); ?>
       <option value="<?php echo $id; ?>" <?php echo $instance['cstv_playlist'] == $id ? 'selected="selected"' : ''; ?> ><?php echo $title; ?></option>
       <?php endforeach; ?>
    </select>
</p>
...
<p>
	...
	<input class="widefat" id="<?php echo $this->get_field_id('cstv_popup_text'); ?>" name="<?php echo $this->get_field_name('cstv_popup_text'); ?>" type="text" value="<?php echo isset($instance['cstv_popup_text']) ? $instance['cstv_popup_text'] : 'Watch Videos Now'; ?>" />
</p>
